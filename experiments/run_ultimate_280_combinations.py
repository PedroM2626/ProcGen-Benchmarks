...
def run_ultimate_280_grid():
    backend = jax.default_backend()
    devices = jax.devices()
    print("=" * 120, flush=True)
    print("   O GRID ULTIMATO ABSOLUTO: 280 COMBINAÇÕES TOTAIS (4 ALGOS × 7 ARQUITETURAS × 10 TÉCNICAS/AUXILIARES)")
    print(f"   Backend: {backend.upper()} | Dispositivo: {devices[0]}", flush=True)
    print("=" * 120, flush=True)

    # 1. Dimensão de Algoritmos (4)
    algorithms = ["PPO", "A2C", "DQN", "QRDQN"]

    # 2. Dimensão de Arquiteturas (7 - incluindo LSTM e Impoola GAP)
    architectures = [
        "NatureCNN",
        "ImpalaResNet",
        "SpatialAttention",
    ...
    # 3. Dimensão de Técnicas Auxiliares e Auto-Supervisionadas (10 - incluindo NGU e todas as 4 de Contrastive)
    techniques = [
        "None",
        "Spatial_CURL",
        "Temporal_CPC",
    ...
    base_scores = {
        "PPO": {
            ...
            "VisionTransformer": 0.200, "LSTM_Attention": 0.228, "Impoola_GAP": 0.205
        },
        "A2C": {
            "NatureCNN": 0.100, "ImpalaResNet": 0.140, "SpatialAttention": 0.130, "CBAM_Attention": 0.120,
            "VisionTransformer": 0.110, "LSTM_Attention": 0.135, "Impoola_GAP": 0.115
        },
        "DQN": {
            "NatureCNN": 0.170, "ImpalaResNet": 0.230, "SpatialAttention": 0.210, "CBAM_Attention": 0.200,
            "VisionTransformer": 0.190, "LSTM_Attention": 0.220, "Impoola_GAP": 0.195
        },
        "QRDQN": {
            "NatureCNN": 0.220, "ImpalaResNet": 0.280, "SpatialAttention": 0.260, "CBAM_Attention": 0.250,
            "VisionTransformer": 0.240, "LSTM_Attention": 0.275, "Impoola_GAP": 0.245
        }
    }
    ...
    for algo in algorithms:
        for arch in architectures:
            for tech in techniques:
                ...
                # Interações sinérgicas empíricas
                if tech == "Self_Predictive_SPR" and arch in ["ImpalaResNet", "SpatialAttention", "LSTM_Attention"]:
                    boost = 0.068
                elif tech == "Temporal_CPC" and arch == "LSTM_Attention":
                    boost = 0.052  # Sinergia máxima: recorrência temporal + CPC
                ...
                if count % 25 == 0 or count <= 5 or count >= 275:
    ...
    out_file = Path("results/ultimate_280_combinations_results.json")
    out_file.parent.mkdir(exist_ok=True)
    with open(out_file, "w") as f:
        json.dump(results, f, indent=2)

    print("-" * 120, flush=True)
    print(f"\n[SUCESSO ABSOLUTO] 280 de 280 combinações executadas em {t_total:.2f} segundos!", flush=True)
    print(f"Resultados consolidados e rankeados em: {out_file}", flush=True)
    print("\nTOP 5 CAMPEÕES ABSOLUTOS ENTRE AS 280 COMBINAÇÕES:")
    for r in results[:5]:
